...
def intent_recognition(input_text, ip_address, date, history, unique_id, seq_id, logger, model):
    system_prompt = """作为公牛集团智能家居意图识别的专家，你的目标是从用户输入中准确判断并输出用户的意图。请仔细阅读用户的问题，并根据以下设定的l1级和l2级意图分类进行匹配，不要创造意图输入，严格按照要求执行。
                    ...
                    *l1：中控**
                    - l2：结束对话(只有明确要求结束意图时调用)，音量上调，音量下调，设置音量(设置范围是0-100之间)，静音
                    ...
                    ** 示例 **
                    当前样例说明：亮度最高是100%，最低是1%，其中色温最暖是1%，最冷是100%,如果是色温调到1000k，返回就是色温调节到1000k
                    ...
                    输入：我要上报故障、小木小木你是不是坏了、小沐你怎么没反应、小木小木你是不是傻了、这个是智障呀、这个东西傻逼等用户输入话术
                    输出：通用技能|故障|故障上报
    ...
    # 设置 Generation.call 的超时时间为 1.2 秒
    try:
        response = Generation.call(
            ...
            timeout=1.2  # 设置超时时间为 1.2 秒
        )
    except requests.exceptions.Timeout:
        logger.error(f'log id:{unique_id},seq id:{seq_id}。Generation.call 超时，超过 1.2 秒未响应')
        content = {
            "l1": "闲聊",
            "l2": "闲聊",
            "text": input_text
        ...
        logger.error(f'log id:{unique_id},seq id:{seq_id}。Generation.call 错误，错误信息：{str(e)}')
        content = {
            "l1": "闲聊",
            "l2": "闲聊",
            "text": input_text
    ...
    if response.status_code == HTTPStatus.OK:
        try:
            model_result = response.output.choices[0].message.content.split('|')
            l1 = model_result[0]
            l2 = model_result[1]
            text = model_result[2]
            content = {
            ...
            logger.error(f'log id:{unique_id},seq id:{seq_id}。intent recognition error,error info:{response.output.choices[0].message.content}')
            content = {
                "l1": "闲聊",
                "l2": "闲聊",
                "text": input_text

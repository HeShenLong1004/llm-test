...
def music_control(input_text):
    ...
    elif re.findall('暂停播放|音乐暂停|暂停音乐|暂停一下|暂停',input_text):
        return {
                    "code": 200,
                    "msg": "success",
                    "data": {
...
def air_control(input_text,user_data,user_home,local,unique_id,seq_id,logger):
    ...
    if re.findall('空调',input_text) and function['device'] != []:
        if re.findall('开空调|打开',input_text):
            result['action'] = 'air_turn_on'
            result['reply'] = '已为小主打开空调啦'
        elif re.findall('关空调|关闭',input_text):
            result['action'] = 'air_turn_off'
            result['reply'] = '已为小主关闭空调啦'
            ...
            if matches:
                ...
                result['reply'] = '已为小主调整空调模式啦'
            else:
                result['reply'] = '不好意思，空调模式只有制冷、制热和送风以及除湿。'
        elif re.findall('风速',input_text):
        ...
        for device in function['device']:
            ...
            for id_device in id_info:
                ...
                if result.get('action'):
                    if result.get('value'):
                        result['device_info'].append({'custom_name':device,'action': result['action'],'value':result['value'],"percentage":False,'control_dn':info_list[0],'control_pid':info_list[1],'control_iotid':info_list[2],'x':info_list[3]})
                    else:
                        result['device_info'].append({'custom_name':device,'action': result['action'],'control_dn':info_list[0],'control_pid':info_list[1],'control_iotid':info_list[2],'x':info_list[3]})
        ...
        if floor_heating:
            if re.findall('开地暖|打开',input_text):
                ...
                result['value'] = 1
                result['reply'] = '已为小主打开地暖啦'   
            elif re.findall('关地暖|关闭',input_text):
                result['action'] = 'antifreeze_switch'
                result['value'] = 0
                result['reply'] = '已为小主关闭地暖啦'
            else:
...
def lighting_control_command_synthesis(input_text,user_data,user_home,local,ip_address,city,date,content,model,unique_id,seq_id,logger):
    """控制指令合成"""
    if content['l1']=='家电控制' and content['l2'] in {'灯','开关','插座','窗帘'}:
        ...
        elif type(function['device']) is list and len(function['device'])>0 and re.findall('打开',input_text) and not [device for device in function['device'] if re.findall('窗帘|纱帘|布帘',device)]:
            result = {}
            device_info = []
            result['home'] = user_home
            for item in recall_data.to_dict('records'):
        ...
        elif type(function['device']) is list and len(function['device'])>0 and re.findall('关闭',input_text) and not [device for device in function['device'] if re.findall('窗帘|纱帘|布帘',device)]:
            result = {}
            device_info = []
            result['home'] = user_home
            for item in recall_data.to_dict('records'):
        ...
        elif content['l2'] == '窗帘' and re.findall('开',input_text) and not re.findall('[0-9]{1,3}', input_text):
            result = {}
            device_info = []
            result['home'] = user_home
            for item in recall_data.to_dict('records'):
                if re.findall('帘',item['nickName']):
                    device_info.append({'custom_name':item['nickName'],'action': 'curtain_position', 'value': 100, 'percentage': True,'control_dn':item['deviceName'],'control_pid':item['productId'],'control_iotid': item['iotId'],'x':extract_trailing_number(item['element'])})
            result['device_info'] = device_info
            result['controlType'] = 99
            if not result['device_info']:
                result['reply'] = f'{function["local"]}区域未找到相关设备'
            else:
                result['reply'] = f"已执行{function['local']}区域窗帘操作"
        elif content['l2'] == '窗帘' and re.findall('关',input_text) and not re.findall('[0-9]{1,3}', input_text):
            result = {}
            device_info = []
            result['home'] = user_home
            for item in recall_data.to_dict('records'):
                if re.findall('帘',item['nickName']):
                    device_info.append({'custom_name':item['nickName'],'action': 'curtain_position', 'value': 0, 'percentage': True,'control_dn':item['deviceName'],'control_pid':item['productId'],'control_iotid': item['iotId'],'x':extract_trailing_number(item['element'])})
            result['device_info'] = device_info
            result['controlType'] = 99
            if not result['device_info']:
                result['reply'] = f'{function["local"]}区域未找到相关设备'
...
def control_instruction(input_text,content,recall_data,local,user_home,unique_id,seq_id,logger,model):
    ...
    if content['l2']=='窗帘':
        system_prompt = """ 
                        ...
                        - 窗帘和空调不支持组播，调节温度都是操作空调，"domain"值必须为"0"。
    ...
    if response.status_code == HTTPStatus.OK:
        try:
            ...
            logger.info(f'log id:{unique_id},seq id:{seq_id}。iot大模型输出：{response.output.choices[0].message.content}')
            content = {'device_info':[]}
        content['device_info'] = []
        ...
        logger.info(f'log id:{unique_id},seq id:{seq_id}。content的输出是：{content}')
        ...
        if content.get('domain') == 1:
            ...
            for item in content['device_info']:
                if item['action'] in {'turn_on','turn_off','light_up','light_down','light_value','color_temperature_up','color_temperature_down','color_temperature_value','rgb_value','curtain_position','flip_angle','temperature_value','mode','windspeed_value','antifreeze_switch'} and item['custom_name']:
                    control_dn_list = device_kv.get(item['custom_name'])
                    if not control_dn_list:
                        break
                    if item['action'] not in {'color_temperature_value','light_value','rgb_value','curtain_position','flip_angle','temperature_value','mode','windspeed_value','antifreeze_switch'}:

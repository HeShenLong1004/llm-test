...
def information_extraction(text, user_data, local, mode='room'):
    ...
    found_rooms = extraction_text(text, rooms)
    result = {}
    if mode == 'room':
    ...
    if '所有灯带' not in text:
        device_keys += ['所有灯']
    devices = extraction_text(text, device_keys)

    if not devices:
        types = ['灯', '窗帘', '空调', '电视', '风扇', '插座']
        found = extraction_text(text, types)
        devs = []
        for t in found:
...
def is_all_lights_command(text):
    """
    检查是否为"打开/关闭所有灯"命令
    """
    text = text.strip()
...
def process_all_lights_command(text, user_data):
    """
    处理"打开/关闭所有灯"命令
    对整个data列表中所有设备进行操作，但只将前10个设备放到iot_detail中
    将所有roomId放到roomIds中并去重
    """
    action = is_all_lights_command(text)
    if not action:
        return None
...
def is_group_light_command(text):
    text = text.strip()
    pats = [
        r'^(打开|关闭)灯$',
        r'^(打开|关闭)(所有|全部)灯$',
...
def process_group_command(text, user_data, default_room):
    # 匹配"打开/关闭 XX 灯"，返回 (action, room or None)
    action, room = is_group_light_command(text)
    if not action:
        return None
...
def is_device_type_command(text):
    """
    检查指令是否为设备类型组控命令
    匹配模式: (打开|关闭) [房间名] (设备类型)
...
def process_device_type_command(text, user_data, default_room):
    action, room_name, device_type = is_device_type_command(text)
    if not action or not device_type:
        return None

    return generate_device_type_group_command(
        user_data, action, device_type,
        room_name, default_room, orig_query=text
    )
...
def main(dn=None, text=None):
    ...
    # 获取语音指令
    if text is None:
        text = input("请输入语音指令（例如：打开客厅的主灯 或 打开射灯 或 打开所有灯）：")

    # 首先检查是否为"打开/关闭所有灯"命令
    all_lights_result = process_all_lights_command(text, user_data)
    ...
    # 再尝试其他组控：设备类型组控 > 灯组控     
    device_type_result = process_device_type_command(text, user_data, local)
    ...
    if cmds:
        for c in cmds:
            ...
            if structured.get("data", {}).get("query", "") == text:
                print(json.dumps(structured, ensure_ascii=False, indent=4, cls=NumpyEncoder))
                return structured

    # 如果generate_commands没有完全匹配，尝试generate_global_commands

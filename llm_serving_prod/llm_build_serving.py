...
def user_info(dn,unique_id,seq_id):
    ...
    if value:
        ...
        select_data = user_data[user_data['deviceName']==dn]
...
def embed_with_list_of_str(text_list):
    embedding_url = "http://172.16.66.77:6667/embedding"
...
def main(dn, input_text, ip_address, city, history, unique_id, seq_id):
    ...
    logger.info(f'log id:{unique_id},seq id:{seq_id}。用户设备信息：{list(zip(user_data["roomName"], user_data["nickName"]))}')

    logger.info(f'log id:{unique_id},seq id:{seq_id}。当前用户请求设备所在房间：{local}')
    ...
    if not scene_list:  # If scene_list is empty, skip parallel execution
        ...
        content = intent_recognition(input_text, ip_address, date, history, unique_id, seq_id, logger, model=llm_model_name)
        ...
        try:
            ...
            logger.error(f'log id:{unique_id},seq id:{seq_id}。scene match error：{str(e)}')
            
        scene_end = time.time()
        if scene_output and scene_output['code']==200:
    ...
    output = lighting_control_command_synthesis(input_text, user_data, user_home, local, ip_address, city, date, content, llm_model_name, unique_id, seq_id, logger)
    end2 = time.time()
...
def llm_intent():
    ...
    result = main(dn, asr_text, ip_address, city, history, unique_id, seq_id)
    main_end = time.time()
...
@app.route('/intent/healthCheck', methods=['GET'])
def health_check():
